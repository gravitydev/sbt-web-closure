...
object ClosureJsPlugin extends AutoPlugin {
  ...
  def compileClosureJs: Def.Initialize[Task[Seq[File]]] = Def.task {
    ...
    val closureJsFiles = (closureLibraryDirectory.value ** ("*.js" -- "*_test.js")).get

    println(closureJsFiles.get)
            ...
            if (err != null) {
              errs.add( 
                new LineBasedProblem(
                  ...
                  //source
                  sources.find(_.getAbsolutePath == err.sourceName).getOrElse(sys.error("Couldn't find source for " + err.sourceName))
                )
              )
            }
          }
